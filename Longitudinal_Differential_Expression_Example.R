...
##Make sure your working directory is set to the where the contents of the Github repository were downloaded / cloned.
...
##lmms and betr discontinued in public repositories during the course of the study, but working versions provided here in this github folder
...
##In the UPS1 Mix datasets, several different types of longitudinal trends and trend differences exist for the spike-in proteins within a single dataset.
...
for(m in 1:length(method.list)){
  set.seed(1)
  temp_pAUC<-getAUC(results = de_results[[m]], reference.prot = rownames(data), spike.prot = ups_spike)
