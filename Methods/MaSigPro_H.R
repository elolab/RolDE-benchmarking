...
getDE<-function(data, sample1, sample2){
  ...
  design <- make.design.matrix(des, degree = nr.timepoints-1) 
  fit <- p.vector(data.temp, design, Q = 1, MT.adjust = "BH", min.obs = thres) 
    
  tstep <- tryCatch({
    T.fit(fit, step.method = "backward", alfa = 0.05, min.obs = thres)
  }, error = function(e) {
    T.fit(fit, step.method = "forward", alfa = 0.05, min.obs = thres)
  })
  p=tstep$sol
